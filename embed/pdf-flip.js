        ...
        for (var i = 0; i < pages.length; i++) {
            PDFViewerApplication.pdfDocument.getPage(pages[i]).then(function (page) {
                ...
                page.render(renderContext).promise.then(function () {
                    ...
                    destinationCanvas.addEventListener('click', function (event) {
                        // Get the click position relative to the canvas
                        var rect = destinationCanvas.getBoundingClientRect();
                        var x = event.clientX - rect.left;

                        // Determine if the click was on the left or right half of the canvas
                        if (x < rect.width / 2) {
